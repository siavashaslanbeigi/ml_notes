...
\begin{figure}[ht]
    ...
    \begin{subfigure}[ht]{\linewidth}
        \centering
        \includegraphics[scale=0.45]{images/grad_des/gd4_slow.pdf}
...
%  In the case of linear regression, we can actually carry out this optimization analytically! Let's go through the math.

% Recall that for linear regression (see \eqref{linreg-eq:multi-costfunc-mat} and \eqref{linreg-eq:costfunc-nbala})
% \begin{align*}
%     J(\theta)
%         &= \frac{1}{2m}(X\theta - Y)^T(X\theta - Y)\\
%     \nabla J(\theta)
...
%     \frac{df}{d\alpha}
%         &=-\nabla J\cdot \nabla J(\theta - \alpha \nabla J) \\
%         &= -\frac{1}{m}[X^T(X\theta - Y)]^T\frac{1}{m}X^T(X(\theta - \alpha \nabla J)  - Y)
